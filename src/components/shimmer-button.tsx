...
export function ShimmerButton({
  ...
  return (
    <Comp
      ...
      whileTap={{ scale: 0.97 }}
      className={`group relative inline-flex items-center justify-center gap-2 overflow-hidden rounded-full bg-or-500 px-8 py-4 text-sm font-semibold uppercase tracking-wider text-bordeaux-950 shadow-lg shadow-or-500/20 transition-shadow hover:shadow-xl hover:shadow-or-500/40 ${className}`}
    >
      {/* Shimmer sweep */}
...
export function BorderAnimateButton({
  ...
  return (
    ...
    >
      ...
      {/* Inner background */}
      <span className="relative flex items-center justify-center gap-2 rounded-full bg-cream px-8 py-4 text-sm font-semibold uppercase tracking-wider text-bordeaux-900 backdrop-blur-sm transition-colors group-hover:bg-cream/90">
        {children}
      </span>
    </a>
  );
